...
<div id="app">
    @include('back.administrasi.sidebar.blog')
    <div id="main">
        <header class="mb-3">
                                ...
                                <div class="card-body">
                                    <div class="card-header">
                                        <div class="card-head-row" style="margin-left: -20px">
                                            <a href="{{route('blog.create')}}" class="btn btn-info btn=sm ml-auto"> <i class="bi bi-plus-circle" style="margin-right: 4px"></i>Tambah Data</a>
                                        </div>
                                    </div>
                                                        ...
                                                        @forelse ($blog as $row)
                                                            @if ($row->delete == 'N')
                                                                <tr>
                                                                    <td>{{ $row->judul}}</td>
                                                                    <td>{{ $row->penulis}}</td>
                                                                    <td>{{ $row->kategori_blog->nama_kategori ?? ''}}</td>
                                                                    <td>
                                                                        @if ($row->is_active == '1')
                                                                            Diterbitkan
                                                                        @else
                                                                    ...
                                                                    <td class="text-bold-500"><a href="{{route('blog.edit', $row->id) }}" class="btn icon btn-primary"><i class="bi bi-pencil"></i></a>
                                                                        <br>
